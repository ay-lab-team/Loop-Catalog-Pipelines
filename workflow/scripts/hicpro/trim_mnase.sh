...
IFS=$'\t'
samplesheet="results/samplesheets/hicpro/mnase.reprocess.hicpro.samplesheet.unmerged.all_batches.without_header.tsv"
...
for fq in *.fastq.gz; do
    ...
    zcat ${id}.fastq.gz | awk '{ if(NR%2==0) {print substr($1,1,50)} else {print} }' | gzip > "trim/${id}_trimmed.fastq.gz"
done
