...
if [ -f "${sample_info[4]}" ]; then
    ...
    if [ -f "${sample_info[2]}" ]; then
        ...
        hp_file_s="results/peaks/overlaps/05_10_2023/${sample_name}.hp.sorted.bed"
        hp_file_slop="results/peaks/overlaps/05_10_2023/${sample_name}.hp.sorted.slop.bed"
        outfile="results/peaks/overlaps/05_10_2023/${sample_name}.overlaps.chipseq.hichip_peaks.$genome.txt"
        outfile_no_dups="results/peaks/overlaps/05_10_2023/${sample_name}.chipseq.hichip_peaks.uniq.$genome.txt"
        ...
        bedtools intersect -wb -a $cs_file_s -b $hp_file_slop -sorted > $outfile
        awk -F["\t"] '{print $4"\t"$5"\t"$6}' $outfile | sort -k1,1 -k2,2n -k3,3n -u > $outfile_no_dups
    ...
    if [ -f "${sample_info[3]}" ]; then
        ...
        f_file_s="results/peaks/overlaps/05_10_2023/${sample_name}.f.sorted.bed"
        f_file_slop="results/peaks/overlaps/05_10_2023/${sample_name}.f.sorted.slop.bed"
        outfile="results/peaks/overlaps/05_10_2023/${sample_name}.overlaps.chipseq.fithichip.$genome.txt"
        outfile_no_dups="results/peaks/overlaps/05_10_2023/${sample_name}.chipseq.fithichip.uniq.$genome.txt"
        ...
        bedtools intersect -wb -a $cs_file_s -b $f_file_slop -sorted > $outfile
        awk -F["\t"] '{print $4"\t"$5"\t"$6}' $outfile | sort -k1,1 -k2,2n -k3,3n -u > $outfile_no_dups
