...
#SBATCH --job-name=interactions_to_longrange.fithichip_hp

set -euo pipefail
IFS=$'\n\t'

# to run the code from bash use:
# bash workflow/scripts/visualizations/longrange/interactions_to_longrange.fithichip_hp.qarray.sh 3

# to run the code from Slurm use:
# sbatch --array=3 workflow/scripts/visualizations/longrange/interactions_to_longrange.fithichip_hp.qarray.sh 

# to run ALL samples with slurm use:
# sbatch --array=1-2868 workflow/scripts/visualizations/longrange/interactions_to_longrange.fithichip_hp.qarray.sh 
...
info=$(sed -n ${SLURM_ARRAY_TASK_ID}p workflow/scripts/visualizations/samplesheets/samplesheet.fithichip_hp.wc.txt)
