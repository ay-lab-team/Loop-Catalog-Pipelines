...
#PBS -N run_fithichip_loopcalling_L25
#PBS -V

# example run:
# 1) qsub -t <index1>,<index2>,... workflow/scripts/run_fithichip_loopcalling_L25.sh
# 2) qsub -t <index-range> workflow/scripts/run_fithichip_loopcalling_L25.sh
# 3) qsub -t <index1>,<index2>,<index-range1> workflow/scripts/run_fithichip_loopcalling_L25.sh
# 4) qsub -t <index-range1>,<index-range2>,... workflow/scripts/run_fithichip_loopcalling_L25.sh
# 5) qsub -t <any combination of index + ranges> workflow/scripts/run_fithichip_loopcalling_L25.sh
...
# print start message
echo "Started: run_fithichip_loopcalling_L25"
...
if [ $peak_mode -eq 1 ]; then
    if [ -f "${sample_info[2]}" ]; then
        ...
        outdir_L25="${PBS_O_WORKDIR}/biorep_merged/results/loops/fithichip/${sample_name}_hichip-peaks.peaks/L25/"
        mkdir -p $outdir_L25
    else
...
if [ $peak_mode -eq 2 ]; then
    if [ -f "${sample_info[3]}" ]; then
        ...
        # make the output directory
        outdir_L25="${PBS_O_WORKDIR}/biorep_merged/results/loops/fithichip/${sample_name}_fithichip.peaks/L25/"
        mkdir -p $outdir_L25
    else
...
if [ $peak_mode -eq 3 ]; then
    if [ -f "${sample_info[4]}" ]; then
        ...
        # make the output directory
        outdir_L25="${PBS_O_WORKDIR}/biorep_merged/results/loops/fithichip/${sample_name}_chipseq.peaks/L25/"
        #outdir_L25="${PBS_O_WORKDIR}/results/pieqtl_ncm_rep_combined_donorwise/fithichip/${sample_name}/L25/"
        mkdir -p $outdir_L25
    else
...
configfile_L25="${outdir_L25}configfile_L25"
...
# print end message
echo
echo "Ended: fithichip loop calling for L25"
echo
