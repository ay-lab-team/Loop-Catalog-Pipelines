...
#PBS -N run_fithichip_loopcalling_L5_t2t
#PBS -V
...
# print start message
echo "Started: run_fithichip_loopcalling_L5_t2t"
...
if [ $peak_mode -eq 1 ]; then
    if [ -f "${sample_info[2]}" ]; then
        ...
        outdir_L5="${PBS_O_WORKDIR}/ref_genome/results/loops/fithichip/${sample_name}_hichip-peaks.peaks/L5/"
        mkdir -p $outdir_L5
    else
...
if [ $peak_mode -eq 2 ]; then
    if [ -f "${sample_info[3]}" ]; then
        ...
        # make the output directory
        outdir_L5="${PBS_O_WORKDIR}/ref_genome/results/loops/fithichip/${sample_name}_fithichip.peaks/L5/"
        mkdir -p $outdir_L5
    else
...
if [ $peak_mode -eq 3 ]; then
    if [ -f "${sample_info[4]}" ]; then
        ...
        # make the output directory
        outdir_L5="${PBS_O_WORKDIR}/ref_genome/results/loops/fithichip/${sample_name}_chipseq.peaks/L5/"
        mkdir -p $outdir_L5
    else
