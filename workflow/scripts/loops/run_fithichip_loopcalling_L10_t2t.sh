...
if [ $peak_mode -eq 1 ]; then
    if [ -f "${sample_info[2]}" ]; then
        ...
        outdir_L10="${PBS_O_WORKDIR}/ref_genome/results/loops/fithichip/${sample_name}_hichip-peaks.peaks/L10/"
        mkdir -p $outdir_L10
    else
...
if [ $peak_mode -eq 2 ]; then
    if [ -f "${sample_info[3]}" ]; then
        ...
        # make the output directory
        outdir_L10="${PBS_O_WORKDIR}/ref_genome/results/loops/fithichip/${sample_name}_fithichip.peaks/L10/"
        mkdir -p $outdir_L10
    else
...
if [ $peak_mode -eq 3 ]; then
    if [ -f "${sample_info[4]}" ]; then
        ...
        # make the output directory
        outdir_L10="${PBS_O_WORKDIR}/ref_genome/results/loops/fithichip/${sample_name}_chipseq.peaks/L10/"
        mkdir -p $outdir_L10
    else
