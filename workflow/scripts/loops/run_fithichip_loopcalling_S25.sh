...
#PBS -N run_fithichip_loopcalling_S25
#PBS -V

# example run:
# 1) qsub -t <index1>,<index2>,... workflow/scripts/run_fithichip_loopcalling_S25.sh
# 2) qsub -t <index-range> workflow/scripts/run_fithichip_loopcalling_S25.sh
# 3) qsub -t <index1>,<index2>,<index-range1> workflow/scripts/run_fithichip_loopcalling_S25.sh
# 4) qsub -t <index-range1>,<index-range2>,... workflow/scripts/run_fithichip_loopcalling_S25.sh
# 5) qsub -t <any combination of index + ranges> workflow/scripts/run_fithichip_loopcalling_S25.sh
...
# print start message
echo "Started: run_fithichip_loopcalling_S25"
...
if [ $peak_mode -eq 1 ]; then
    if [ -f "${sample_info[2]}" ]; then
        ...
        outdir_S25="${PBS_O_WORKDIR}/biorep_merged/results/loops/fithichip/${sample_name}_hichip-peaks.peaks/S25/"
        mkdir -p $outdir_S25
    else
...
if [ $peak_mode -eq 2 ]; then
    if [ -f "${sample_info[3]}" ]; then
        ...
        # make the output directory
        outdir_S25="${PBS_O_WORKDIR}/biorep_merged/results/loops/fithichip/${sample_name}_fithichip.peaks/S25/"
        mkdir -p $outdir_S25
    else
...
if [ $peak_mode -eq 3 ]; then
    if [ -f "${sample_info[4]}" ]; then
        ...
        # make the output directory
        outdir_S25="${PBS_O_WORKDIR}/biorep_merged/results/loops/fithichip/${sample_name}_chipseq.peaks/S25/"
        #outdir_S25="${PBS_O_WORKDIR}/results/pieqtl_ncm_rep_combined_donorwise/fithichip/${sample_name}/S25/"
        mkdir -p $outdir_S25
    else
...
configfile_S25="${outdir_S25}configfile_S25"
...
# print end message
echo
echo "Ended: fithichip loop calling for S25"
echo
